...
export async function GET() {
  try {
    ...
    const books = await query<Book>("SELECT * FROM books ORDER BY created_at ASC").catch(() => []);
    const quotes = await query<Quote>("SELECT * FROM quotes ORDER BY created_at ASC").catch(() => []);
